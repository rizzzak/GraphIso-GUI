...
int FullMethod::Iteration(std::vector<std::vector<int>>& bigGraph, std::vector<std::vector<int>>& smallGraph, std::vector<std::vector<int>>& isoMatr, std::vector<int>& isoP)
{
    ...
    if (localQ == smallGraph.size() * smallGraph.size())
        noMorePossibleCombinations = true; //ïîñëåäíÿÿ èòåðàöèÿ ÏÏ
    else
    {
...
void FullMethod::Init(int smallSize, int bigSize, int _goal, int* iterationCounter2, int _iterationLimit)
{
    this->MethodBase::Init(smallSize, bigSize, _goal, iterationCounter2, _iterationLimit);
    noMorePossibleCombinations = false;
}
bool FullMethod::NextPermutation(std::vector<int>& a, int k, int smallSize)
{
    int edge, j, i, tmp;
    edge = k - 1;

    // find j in (k…smallSize-1) where a[j] > a[edge]
    j = k;
    ...
    else
    {
        ...
        // find j in (smallSize-1…i+1) where aj > ai
        j = smallSize - 1;
        while (j > i && a[i] >= a[j]) --j;
